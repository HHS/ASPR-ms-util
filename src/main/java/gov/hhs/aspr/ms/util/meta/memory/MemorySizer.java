package gov.hhs.aspr.ms.util.meta.memory;

public class MemorySizer {

	/**
	 * The InstanceFactory is used to generate new instance of the tested class. The
	 * instance factory must 1)generate a new instance each time its getNext()
	 * method is invoked, 2) not retain the generated instances or any other state
	 * that is not present at the first invocation of the factory's getNext() method
	 * and 3) generate an unspecified number of instances not limited to the
	 * instance count used in the static getAverageInstanceByteCount() method.
	 * 
	 * 
	 */
	...
	/**
	 * Returns the average number of bytes used per instance that is generated by
	 * the given instance factory. The larger the instanceCount, the more accurate
	 * will be the estimate. The instanceCount may be exceeded during the
	 * calculation and serves as a suggested value.
	 * 
	 */
	public static double getAverageInstanceByteCount(InstanceFactory instanceFactory, int instanceCount) {
		/*
		 * We do not advertise the possible run time exception since it is a contrivance
		 * to keep the JVM from doing memory clean up before we can do the accounting.
		 */
		try {
			return _getAverageInstanceByteCount(instanceFactory, instanceCount);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}

	private static double _getAverageInstanceByteCount(InstanceFactory instanceFactory, int instanceCount)
			throws Exception {

		final Object[] instances = new Object[instanceCount];

		// stimulate the garbage collection process and the instance factory
		outterForceGarbageCollection();
	...
	/*
	 * Executes the actual garbage collection calls and uses Thead.yield() to
	...
	private static void innerForceGarbageCollection() throws Exception {
		...
		while ((currentBytesInUse < lastBytesInUse)
				&& (attemptsToRunGarbageCollection < MAX_ATTEMPTS_TO_RUN_GARBAGE_COLLECTION)) {
	 ...
	 * method actually calls the runtime garbage collection and we use this outer
	 * method to help assure that the exiting of the inner method will help the JVM
	 * actually finish up on garbage collection.
	 */
	private static void outterForceGarbageCollection() throws Exception {
